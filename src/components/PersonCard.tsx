...
export function PersonCard({ person, className }: PersonCardProps) {
  ...
  return (
    <Link href={`/person/${slugify(person.name)}`} className="block group mt-4 h-full">
      <Card className={cn("overflow-hidden h-full transition-all duration-300 group-hover:shadow-lg group-hover:-translate-y-1", className)}>
